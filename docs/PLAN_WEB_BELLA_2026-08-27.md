...
| **BELLA-9** Admin, SEO y apps | Sonnet | Título "Frutiferia" sin duplicar + meta desc 152c, product_type es-CL (Paltas/Papas/…), retiro a "24 horas" con horario real (D-5), Judge.me a dieta, VUELVE10 creado y ACTIVO (aprobado), guía desinstalar Smile (G-5) | Admin de Shopify vía navegador (nada del repo) | ⏳ 2026-09-01 — parcial EN VIVO: SEO título+descripción, retiro (mensaje y bloque), VUELVE10 **activo**. Pendiente: tipos es-CL, Smile (G-5), imagen social (G-14). |
...
| G-14 | **Cambiar la imagen que se ve al compartir por WhatsApp** (1 min): hoy sale la foto de cumpleaños con globos. Tienda online → Preferencias → clic en la imagen de «Imagen de redes sociales y SEO» → elegir **hero-desktop** (ya está subida) → Guardar. | 1 min | WhatsApp es EL canal de Frutiferia y el enlace se comparte con esa foto. No se hizo por código para no tocar el `<head>` de todo el sitio por un beneficio menor. | Cuando puedas. |
| G-15 | **Tipos de producto en chileno** (10 min, opcional): renombrar «Aguacates»→Paltas, «Patatas»→Papas, «Calabazas y calabacines»→Zapallos, fusionar «Manteca(s) de frutos secos»→Mantequillas de Frutos Secos; y mover los **12 productos de «Frutas del bosque»** (ninguno es berry: son mandarina, kiwi, manzanas, pera, plátano, piña, mango, maracuyá) al tipo **«Fruta»**, donde ya viven sus hermanos. Listas exactas con handle en el registro de la sesión. Riesgo bajo: las 29 colecciones se explican por TAG, no por tipo. | 10 min | Solo afecta el desplegable del buscador; por eso quedó al final. | Cuando quieras. |
