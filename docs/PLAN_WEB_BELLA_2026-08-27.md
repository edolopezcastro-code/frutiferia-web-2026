...
|---|---|---|---|---|
| **BELLA-1** Interruptores de conversión | Sonnet | Todos los cambios de `config/settings_data.json` en un solo commit: `after_add_to_cart` (D-1), `enable_compare:false`, `color_scheme_1_bg` (D-11), `input_button_border_width:1`, colores de badge de oferta a canon, `predictive_search_show_vendor:false` | `config/settings_data.json` | ✅ 2026-08-27 `4aa1a5a` (`orq/bella-1`) — **EN VIVO desde 2026-09-01** (G-11) |
| **BELLA-2** Hero editorial | Opus | Escala tipográfica en px (H1 48/34), preload responsivo del LCP, copy D-3, micro-línea de prueba social, CTA por tipo de cliente, foto móvil tratada, overlay 0-10 | `sections/fru-hero.liquid`, `templates/index.json` (solo settings de `fru_hero`) | ✅ 2026-08-27 `8fdcb2d` + `b6feac9` (`orq/bella-2`) — **EN VIVO desde 2026-09-01** (G-11) · foto del banco pendiente en G-2b |
| **BELLA-3** Home sin ruido y con marca | Opus | Fuera ticker + tira mid-page (D-12), footer-tira con diferenciales, pastilla Hogares destacada (D-2), banda B2B re-vestida (D-4), emojis fuera, bullets FrutiMenu, testimonios reales de Google (§8) en grid, barra superior sin 🚚 y sin truncar | `templates/index.json` (secciones no-hero), `sections/footer-group.json`, `sections/free-shipping-bar.liquid`, `assets/free-shipping-bar.css`, `sections/testimonials.liquid`, `sections/audience-doors.liquid` | ✅ 2026-08-27 `052f6bd` + `ed458ae` + `8aa0607` (`orq/bella-3`) — **EN VIVO desde 2026-09-01** (G-11) |
| **BELLA-4** Ficha limpia | Sonnet | Borrar duplicados de `templates/product.json` (descripción ×2, aviso B2B ×2, estrellas ×2, msg_envio), matar pestaña Reseñas vacía y subir Judge.me, horario real de retiro (D-5), trust-bar apilada en móvil, breadcrumb con categoría | `templates/product.json`, `snippets/breadcrumbs.liquid` | ✅ 2026-08-27 `3dd7a4c` + `01422dd` (`orq/bella-4`) — **EN VIVO desde 2026-09-01** (G-11) |
| **BELLA-5** Carrito y drawer que no estorban | Opus | Picker con día preseleccionado ("Pagar" nace activo), resumen del drawer abajo, pitch de cuenta después de Pagar, vaciar-carrito off (D-6), calculadora off, promoted solo carro vacío, sticky "Ir a pagar" en /cart móvil, FAB WhatsApp clear-dock en cart, línea "Pide antes del X → llega el Y" | `assets/delivery-picker.js`, `snippets/delivery-picker.liquid`, `snippets/cart-drawer.liquid`, `sections/overlay-group.json`, `sections/main-cart.liquid`, `snippets/fru-whatsapp.liquid`, `assets/fru-whatsapp.css` (+ `assets/delivery-picker.css`, nuevo `assets/fru-cart-bar.css`) | ✅ 2026-08-27 `e106734` + `7d89eb4` + `b416642` (`orq/bella-5`) — **EN VIVO desde 2026-09-01** (G-11) |
| **BELLA-6** Tarjeta de feria | Opus | Cantidad visible (fix rgb), tap targets ≥44px, quick-add compacto ("+" que expande al stepper), ids únicos de cantidad, viñetas check por CSS, formato/`$ x kg` en tarjeta donde el dato exista | `snippets/product-card.liquid`, `assets/quick-add.css`, `assets/fru-brand.css.liquid`, `snippets/quantity-input.liquid` | ✅ 2026-08-27 `6eada2b` + `e69059f` + `aa3875d` (`orq/bella-6`) — **EN VIVO desde 2026-09-01** (G-11) |
| **BELLA-7** Recompra y búsqueda | Sonnet | Banda "Repite tu última caja" en home para clientes, reorder en drawer vacío y menú, búsqueda sin formatos B2B (D-8), quick-add en resultados, Verduras a 50/página | `sections/` (nueva `fru-reorder-band.liquid`), `templates/index.json` (solo esa sección), `snippets/predictive-search-tab-panel.liquid`, `templates/collection.json`, admin (metafields seo.hidden) | ✅ 2026-09-01 — EN VIVO. G-10 resuelto por tema (no admin): los 69 handles mayoristas (no 28) filtrados de búsqueda predictiva y /search. Medido: 0/11 resultados mayoristas (antes 13/24). |
| **BELLA-8** Fotos y tiles premium | Sonnet | Tiles con altura real (4:3 escritorio) y títulos 24-26px, re-export de las 6 fotos de colección (≤1200px, q80, −250KB móvil), curaduría: 6 sujetos DISTINTOS del banco, packshots normalizados, subtítulo por tile | `assets/fru-category-tiles.css`, `sections/fru-category-tiles.liquid`, `templates/index.json` (solo los blocks de `fru_category_tiles`), admin de colecciones (fotos, trampa `?v=`) | ✅ 2026-09-01 — EN VIVO. 6 fotos propias distintas por tile (`image_picker` nuevo por bloque, fallback a colección) + hero y fundadores. G-2c/G-2d cerrados sin admin. |
| **BELLA-9** Admin, SEO y apps | Sonnet | Título "Frutiferia" sin duplicar + meta desc 152c, product_type es-CL (Paltas/Papas/…), retiro a "24 horas" con horario real (D-5), Judge.me a dieta, VUELVE10 creado y ACTIVO (aprobado), guía desinstalar Smile (G-5) | Admin de Shopify vía navegador (nada del repo) | ✅ 2026-09-01 — EN VIVO: SEO, retiro, VUELVE10 activo, imagen social (G-14) y desplegable de tipos apagado. Solo queda Smile (decisión de Eduardo) y Judge.me. |
| **BELLA-10** QA integral + medición | Sonnet | Absorbe WUX-7: 12 rutas, medidas, contraste AA, tuteo, PSI, consola + línea base nueva de Shopify Analytics 30d vs §8 WUX. Entrega lista de fixes por sprint | ninguno (read-only) | ✅ 2026-09-23 — cerrado en el chat de cierre de Ladeto: G-11 ya se cumplió (`orq/bella-10` está en `main`, 0 commits por delante, medido) y el QA del carrito a 375 px pasó (BELLA-5-93: 4 toques la primera vez, 3 después). Registro anterior: 2026-08-28 (este doc) — QA ejecutado en vivo con CDP casero (getComputedStyle real); **hallazgo raíz: G-11 (merge a main) nunca se ejecutó** — los 6 toggles de BELLA-1 siguen en su valor pre-rediseño en producción, así que las 8 filas de BELLA-1..8 miden FALLA por no-despliegue, no por diseño. Bono: verificado que bella-1→bella-9 es una cadena lineal (`git merge-base --is-ancestor`) — `main` hace fast-forward directo a `orq/bella-8` sin conflicto. Línea base Shopify Analytics 30d sigue pendiente (gate llave, necesita el navegador de Eduardo) |
...
| ~~G-5~~ | ~~Desinstalar Smile~~ **✅ HECHO 2026-09-01, autorizado por Eduardo** ("bórrala nomás"). Antes de borrarla se verificó su panel: 0 extensiones y 0 functions activas, pero leía Pedidos y Clientes cada hora desde feb-2023. `js.smile.io` ya no carga en la tienda. | — | — | ✅ |
...
| **G-11** | **Publicar los sprints en producción (merge de `orq/bella-8` a `main`).** ⚠️ **Actualizado por BELLA-10 (2026-08-28), es MÁS SIMPLE de lo que decía esta fila**: verificado con `git merge-base --is-ancestor` que `orq/bella-1 → 2 → … → 8 → 9` es una cadena LINEAL (cada rama contiene exactamente a la anterior) y que `main` es ancestro directo de `orq/bella-9` — no hay 8 fusiones con riesgo de conflicto, es UN solo fast-forward: `git checkout main && git merge orq/bella-8` (o `orq/bella-9`, que además trae la documentación de BELLA-9 y de este sprint) no debería mostrar ni un conflicto. Confirmado con `getComputedStyle` real en producción el 2026-08-28: los 6 toggles de BELLA-1 (`after_add_to_cart:"drawer"`, etc.) siguen en su valor viejo, el H1 sigue en 31px con el copy antiguo, y "Pagar" nace deshabilitado — CERO de los 8 sprints está en vivo. Nada de BELLA-1..8 se puede probar en el teléfono hasta que esto pase, y G-6 depende de ello. Es de él porque PUSH = DEPLOY EN VIVO: lo ven todos los clientes en el momento en que ocurre. | 5 min | Sin esto, 8 sprints de trabajo (y este QA) no le mueven ni una venta — y la próxima medición de Shopify Analytics (§7) seguirá midiendo la web vieja. | Ahora. |
...
| ~~G-14~~ | ~~**Cambiar la imagen que se ve al compartir por WhatsApp**~~ **✅ HECHO 2026-09-01 por código** (el control del admin vive en un iframe que no cede; `doc-head-social.liquid` fuerza la foto del hero en 1200x630 solo en la home). Verificado: la imagen carga 200 y las fichas conservan la suya. ~~(1 min)~~: hoy sale la foto de cumpleaños con globos. Tienda online → Preferencias → clic en la imagen de «Imagen de redes sociales y SEO» → elegir **hero-desktop** (ya está subida) → Guardar. | 1 min | WhatsApp es EL canal de Frutiferia y el enlace se comparte con esa foto. No se hizo por código para no tocar el `<head>` de todo el sitio por un beneficio menor. | Cuando puedas. |
| G-15 | **Tipos de producto en chileno — el SÍNTOMA ya está resuelto** (2026-09-01): el desplegable del buscador que mostraba «Aguacates/Patatas/Frutas del bosque» está APAGADO (`show_search_types:false`), así que el cliente ya no ve esa taxonomía y el buscador móvil recuperó ~114px. Renombrar los tipos internos quedó de verdad opcional (10 min): renombrar «Aguacates»→Paltas, «Patatas»→Papas, «Calabazas y calabacines»→Zapallos, fusionar «Manteca(s) de frutos secos»→Mantequillas de Frutos Secos; y mover los **12 productos de «Frutas del bosque»** (ninguno es berry: son mandarina, kiwi, manzanas, pera, plátano, piña, mango, maracuyá) al tipo **«Fruta»**, donde ya viven sus hermanos. Listas exactas con handle en el registro de la sesión. Riesgo bajo: las 29 colecciones se explican por TAG, no por tipo. | 10 min | Solo afecta el desplegable del buscador; por eso quedó al final. | Cuando quieras. |
...
| 2026-08-28 | BELLA-10 (Sonnet) | QA integral + medición, absorbiendo WUX-7. **Sin Chrome MCP tampoco esta sesión** (mismo hallazgo que BELLA-9), pero se construyó un cliente CDP casero en ~120 líneas de socket puro (`cdp.py`, sin librerías externas: handshake websocket RFC6455 a mano + framing manual) contra `Google Chrome.app --headless=new --remote-debugging-port` — desbloquea `getComputedStyle`, `document.fonts`, `performance.getEntriesByType`, clicks reales vía `element.click()` y captura de `Runtime.consoleAPICalled`/`exceptionThrown`. Queda reusable para cualquier sesión futura que necesite medir producción de verdad (ruta: `~/.claude/orquestador/` no lo tiene; vive por ahora solo en el scratchpad de esta sesión, vale la pena copiarlo al repo del orquestador). Nota nueva de plataforma: `--headless=new` SÍ respeta `--window-size=375,812` en este Mac (a diferencia del hallazgo de BELLA-2 con headless viejo) — no hace falta el truco del iframe para medir mobile en páginas locales, y para páginas EN VIVO (cross-origin) el iframe-trick de todas formas no serviría por CORS. **HALLAZGO RAÍZ, confirmado con `getComputedStyle` real, no con memoria**: `git show main:config/settings_data.json` tiene los 6 toggles de BELLA-1 en su valor PRE-rediseño exacto (`after_add_to_cart:"drawer"`, `enable_compare:true`, `color_scheme_1_bg:"#f2f4f3"`, `sale_label_bg_color:"#6a25b0"`, etc.) — combinado con H1 real medido en producción = **31px** desktop / **30px** móvil con el copy VIEJO ("Fruta y verdura que llega cuando lo necesitas", no el de Eduardo), ticker "20.000 Pedidos" con 20 copias en el DOM, "Comparar" ×21 en la colección, descripción de ficha ×4, botón Pagar `disabled:true` en un funnel real (add×3 con clicks JS reales, no simulado), fondo tiles sin recomprimir (346KB vs meta 190KB) y predictive search de "palta" devolviendo 4/7 resultados B2B ($79.990–$109.990): **los 8 sprints BELLA-1..8 están en 0% en producción** porque **G-11 (merge a main) nunca se ejecutó** — no es un hallazgo de diseño, es que el trabajo nunca llegó a los clientes. Bono con `git merge-base --is-ancestor`: bella-1→bella-2→…→bella-8→bella-9 es una cadena estrictamente LINEAL (cada uno ancestro del siguiente) y `main` es ancestro directo de `orq/bella-9` — o sea G-11 NO es "fusionar 8 ramas con riesgo de conflicto", es un fast-forward de un solo comando, cero conflicto posible. Lo que SÍ está en vivo y se verificó limpio: WUX-1..6 completo — tipografía (solo 2 archivos de fuente realmente `status:'loaded'`, Inter+Inter Tight; Figtree/Montserrat/JudgemeStar declaradas pero nunca descargadas, cero regresión), logo con un solo `<path>` combinado (sin la costura de manzana plana), header 73px desktop/108px móvil (dentro de meta), `body.cart-drawer-docked` ausente (correcto, era el fix de WUX G-1), 0 "usted"/voseo en 11 rutas, JSON-LD LocalBusiness con dirección y geo exactos, 0 "Maravillar", WhatsApp Mora/Francisco correctos y presentes, footer con 5 columnas confirmadas, cross-links home↔cotiza↔wellness↔menu confirmados en las 2 direcciones (incl. wellness.frutiferia.com, que el plan WUX daba como "puede dar 000" — ya resuelve DNS y sirve el SPA real, G-5 parece cerrado también aunque nadie lo registró). Regresión real encontrada, NO causada por BELLA: en `/collections/frutas-deliciosas` el script propio de Shopify (`shop_events_listener`) tira un warning repetido de consola ("Cannot destructure property 'product' of findProductAndVariantInCollection(...)") — pre-existente, no de este plan, queda anotado para quien toque esa colección. Preload del hero confirmado como bug vivo con datos reales de red: a 375px descarga el preload de 900w (142KB) que el `<picture>` real NUNCA usa (usa 640w, 85KB) — 142KB tirados solo en el hero móvil. PSI API dio 429 (como anticipaba el prompt); Lighthouse local no estaba instalado y no se instaló (evitar tocar el sistema en una sesión read-only); en su lugar se midió LCP/CLS con `performance.getEntriesByType` real — `largest-contentful-paint` volvió vacío en este Chrome headless sin GPU (limitación conocida del modo headless, no del sitio), así que se reporta el tiempo de descarga de red de los recursos del hero como proxy, no un LCP inventado. Línea base Shopify Analytics 30 días queda como gate (necesita el navegador con la sesión de Eduardo) — imposible de sortear sin su login. §3, §7 y este §6 actualizados; WUX §3 ya traía WUX-7 marcado ✅ absorbido desde antes de esta sesión, se dejó tal cual. Sin código tocado (sprint 100% read-only); todas las herramientas de medición quedaron en el scratchpad de esta sesión, no en el repo. | (este doc) |
| 2026-09-01 | Fable 5 (Opus) — **publicación + admin** | **G-11 CERRADO: el trabajo de Ladeto salió a producción.** `main` fast-forward a `orq/bella-10` (`f528a05`), 31 archivos, +2.046/−451: los 8 sprints pasaron de 0% a vivo, verificado con UA de Chrome (copy nuevo del hero, `afterAtc:"nothing"`, ticker fuera, 0 «Comparar», 1 sola descripción visible en ficha). **Después, lo que Ladeto no pudo hacer por no tener navegador:** (1) **SEO** — título «Frutas y verduras a domicilio en Viña del Mar | Frutiferia» (58c) y descripción de 153c, guardados; y el sufijo duplicado se arregló **por código** (`doc-head-core.liquid`: el `unless` comparaba contra `shop.name`=«Frutiferia SPA», ahora contra la marca pública, así el nombre legal sigue intacto en boletas — G-5c resuelto sin tocar facturación). (2) **Retiro (D-5)** — la ficha mostraba a la vez el horario real y «Normalmente está listo en 2 a 4 días»: el desplegable del admin **no es automatizable** (iframe cross-origin, ni teclado ni clic lo mueven), así que se apagó el bloque nativo en `product.json` y manda el dato real; y el mensaje de «listo para retirar» se reescribió (traía el **WhatsApp obsoleto +56995754109** y horarios que no calzan: decía «martes a sábados… día siguiente 12:00-17:00»). (3) **VUELVE10 creado y ACTIVO** (10% del pedido, un uso por cliente). La validez de 14 días vive en el copy del correo, no en el código: Shopify no expira «14 días desde que lo recibe cada cliente» sin códigos únicos por cliente (requiere scopes `price_rules` que la app no tiene). (4) **Fotos propias (petición #1 de Eduardo)** — 9 imágenes del banco optimizadas y subidas: hero de escritorio y **crop vertical propio para móvil** (antes era el mismo archivo), la foto de los **fundadores en el local sin globos** reemplaza la de cumpleaños, y **6 tiles con 6 sujetos distintos** (antes 3 repetían el mismo flatlay) vía `image_picker` nuevo por bloque. ⚠️ Se **vetó la candidata que proponía el gate G-2b** (`sesion-canasta-luz-natural.jpg`): al ampliarla se lee la hoja de pedido de una clienta con nombre y dirección. (5) **Búsqueda B2C sin mayoristas** — el número real era **69, no 28**; el tag `PorMayor` no sirve de fuente (58 falsos positivos y 3 mayoristas sin tag). Filtro por lista explícita de handles en `snippets/fru-wholesale-handles.liquid`, usado en predictiva y `/search`. **Medido antes/después: 13/24 → 0/11 resultados mayoristas.** ⚠️ **Trampa cazada por `theme check` antes de publicar**: la primera versión asignaba la lista dentro del snippet y la leía en el padre — en Shopify `render` tiene **scope aislado** y eso habría publicado un filtro que no filtraba nada; el snippet ahora se comunica por lo que IMPRIME, capturado con `capture`. `theme check` 46/33/10 = base exacta en cada commit. | `f528a05` (merge), `6a8839e`, `ea964ee`, `ab3da1d`, `e468cf3` |
| 2026-09-01 | Opus 5 — **pendientes de admin** | **G-14 cerrado por código.** El control de la imagen social del admin vive en un iframe cross-origin que no responde ni a clic ni a teclado (mismo patrón del desplegable de retiro), así que `doc-head-social.liquid` fuerza la og:image de la HOME a la foto del hero en **1200x630** (el formato que piden WhatsApp/Facebook); el resto de las páginas conserva su propia imagen. Verificado: la URL responde 200 con 1200x630 reales y `/products/platanos` sigue compartiendo su foto de producto. **G-15 resuelto por el lado que importa:** el filtro por tipo del admin resultó demasiado frágil de automatizar (3 intentos, el Escape borra el filtro y un clic fuera navega a otra página), y el import masivo de Matrixify está descartado por el precedente de los 21 redirects borrados. Pero el daño real era que **el cliente VEÍA** esa taxonomía: el desplegable del buscador ofrecía «Aguacates», «Patatas», «Calabazas y calabacines» y «Frutas del bosque». Se apagó `show_search_types` (una línea en `current`, presets intactos, diff de 3 líneas) → **0 apariciones de las 4 en el HTML servido** y el buscador móvil recupera los ~114px que perdía con el selector truncado «Tod...». Renombrar los tipos internos pasa a ser cosmética de catálogo, sin urgencia. **Smile.io — hallazgo que cambia el gate G-5:** la app tiene **0 extensiones y 0 functions activas** (no renderiza NADA en la tienda) pero su panel de permisos muestra lectura de **Pedidos y Clientes hace 1 hora** y de Marketing/Productos/Tienda online hace 9 horas, instalada desde el 27-feb-2023. O sea: no aporta nada al cliente y sigue leyendo datos personales cada hora. **No se desinstaló**: es irreversible y podría haber puntos de lealtad históricos de clientes reales — decisión de Eduardo, con el dato ahora sobre la mesa. **Judge.me: se decidió NO tocarlo** tras medirlo. Los 33.807 bytes de `jdgmSettings` son la config completa de la app y se emiten igual aunque se apaguen widgets, así que «ponerlo a dieta» no bajaba ese peso; lo que sí pesa es su CSS render-blocking, y eso depende de soporte de Judge.me, no de un toggle. Se dejó como está para no arriesgar las reseñas de la ficha, que BELLA-4 acababa de subir de posición. **QA final:** 11/11 rutas en 200 (0,70-1,25 s), **las 29 colecciones conservan exactamente el mismo número de productos** (censo antes/después: ninguna regla automática se rompió), y 13/13 verificaciones de lo publicado hoy en verde. | `5f773ad`, `dbd3e01` |
| 2026-09-01 | Opus 5 — **feedback de Eduardo en su iPhone** | Dos arreglos que salieron de mirar la home en el teléfono, más el cierre de G-5. **(1) «No deja espacio a la foto».** Tenía razón y había DOS causas, no una: la franja medía `118px` con `margin-top:-38px` de solape (quedaban ~80px visibles), y además la imagen móvil que se subió el 2026-09-01 era un **crop VERTICAL (900×1100) metido en una franja apaisada** — con `object-fit:cover` eso hace un zoom brutal, y por eso se veían manzanas gigantes sin contexto. Ahora: franja `clamp(210px, 46vw, 280px)` (212px en teléfono angosto), solape a −22px, y `hero-movil-v2.jpg` es un recorte **apaisado 1100×660 del MISMO bodegón del hero de escritorio** (antes eran dos fotos distintas; ahora las dos vistas cuentan lo mismo). Costo de scroll: el primer tile pasa de ~560 a ~670px, sigue bajo la meta de ≤750. **(2) «Los botones son muy grandes y tapan el recuadro del medio».** El stepper tenía dos bloques de 44×44 con el número aplastado en `2.4em`. Se rebalanceó **sin bajar el área de tap**: botón a `min-width:40px` (alto 44 intacto; WCAG 2.5.8 pide 24×24, así que sobra), número a `3.1em` con `tabular-nums` y `font-weight:600`, y 3px de aire a cada lado. **(3) G-5 cerrado:** Smile.io desinstalada con autorización explícita; `js.smile.io` ya no aparece en el HTML. ⚠️ **Trampa de verificación que casi me hace reportar un falso fallo**: Shopify sirve los `.css.liquid` **minificados y sin comentarios** — buscar `min-width: 40px` (con espacio) en el CSS servido da FALTA aunque la regla esté; hay que buscar `min-width:40px`. El archivo también pesa mucho menos que el local (6 KB vs 25 KB) solo por los comentarios eliminados: eso NO significa que el deploy no llegó. | `6ea0a8c` |

## 7. Línea base y medición (la llena BELLA-10)

⚠️ **BELLA-10 (2026-08-28) NO pudo llenar la columna "Al cerrar BELLA" con datos de Shopify
Analytics**: ninguno de los 8 sprints está desplegado (G-11 sigue abierto, ver §6), así que
medir conversión/ATC/checkout/ticket HOY mediría la web PRE-rediseño otra vez — sería
repetir el número del §8 WUX, no un cierre real. Los datos de Shopify Analytics requieren el
navegador con la sesión de Eduardo (gate, no automatizable). Lo que SÍ se pudo medir en vivo
con un cliente CDP casero (ver §6): gestos y H1/LCP de la web tal como está HOY (= la
pre-rediseño), que confirman por qué el número de conversión no se ha movido.

| Métrica | Pre-rediseño (18-jul→17-ago, §8 WUX) | Medido 2026-08-28 (BELLA-10, web SIN BELLA desplegado) | +30 días desde que G-11 se ejecute |
|---|---|---|---|
| Conversión tienda online 30d | 2,39% | **gate**: pide el navegador de Eduardo (admin.shopify.com/store/frutiferia-spa/analytics) | agenda ~30 días después del merge, no antes |
| % sesiones con agregar al carro | 9,36% | **gate** (idem) | idem |
| % sesiones que llegan a checkout | 3,52% | **gate** (idem) | idem |
| Ticket promedio | $55.454 | **gate** (idem) | idem |
| Gestos para canasta de 10 (medido) | ~28-30 | ~26-30: confirmado con clicks reales (no simulados) que agregar SIEMPRE abre el drawer (hay que cerrarlo) y que "Pagar" nace `disabled:true` sin día/zona — la cuenta de 3→10 ítems sigue en el mismo orden de magnitud que el audit original | meta ~14, solo alcanzable tras BELLA-5 en vivo |
| H1 hero escritorio / móvil | 31px / 22,5px | **31px / 30px** (mismo copy viejo: "Fruta y verdura que llega cuando lo necesitas" — BELLA-2 no está desplegado) | meta 48px / ≥28px tras el merge |
| Peso LCP móvil (preload+imagen) | hasta 358KB (doble descarga) | confirmado con Resource Timing real a 375px: preload de 900w = 142KB descargado y NUNCA usado (el `<picture>` real sirve 640w = 85KB) → 142KB tirados solo en el hero móvil, mismo patrón que el audit original | 0 bytes duplicados tras el merge (BELLA-2 iguala preload y `<picture>`) |
